...
while(1)
	{
	      		...
	      		if (msg_receive($queue_s,$msgtype_receive ,$msgtype_erhalten,$maxsize,$daten,$serialize_needed, $option_receive, $err)===true) 
			{
			 	...
			 	list($lab_id, $lab_session_id, $lab_session_ip , $cmd, $status) = explode(":", $daten );
			 	echo "	<br>Lab id :".$lab_id ;
			 	echo "	<br>Lab session  id :".$lab_session_id ;
			 	echo "	<br>Lab session  ip :".$lab_session_ip ;
			 	echo "	<br>cmd :".$cmd ;
				...
				update_lab_session_ip_status($lab_id  , $lab_session_id , $lab_session_ip ,  $status) ;
		
				/*
				$rtv['lab_id']=$lab_id ;
				$rtv['lab_session_id']=$lab_session_id ;
				$rtv['status']=$status  ;
				$message = json_encode($rtv, JSON_FORCE_OBJECT , 512); 
