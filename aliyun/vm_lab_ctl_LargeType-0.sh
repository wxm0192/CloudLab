...
vm_start(){
	if [ $# -lt  3 ] ; then
		echo ' usage : ./vm_start    Images-name  IP time_limit'
		exit -1;
	fi
	...
	#Allocate Public IP  , Without allocate in this script 
...
if [ $# -lt  6 ] ; then
        echo ' usage : ./vm_lab_ctl.sh  Operation_type(new, del )  Images-name  Tag   Net_work   IP time_limit'
        exit -1;
fi
